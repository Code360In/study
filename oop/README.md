...
- isinstance 함수
   ...
   *  isinstance(첫 번째 파라미터에는 검사할 인스턴스의 이름, 두 번째 파라미터에는 기준 클래스의 이름)
- issubclass 함수
   * 한 클래스가 다른 클래스의 자식 클래스인지를 알려주는 함수입니다.
   * issubclass(첫 번째 파라미터로 검사할 클래스의 이름, 두 번째 파라미터에는 기준이 되는 부모 클래스의 이름)
- 오버라이딩 : 부모에게 상속된것을 맞게 사용하는것
- super() : 부모 클래스의 메소드를 호출 가능 
- 다중 상속 
   * 부모클래스끼리 같은 이름의 메소드를 갖지 않도록 하기
   * 같은 이름의 메소드는 자식크래스에서 오버라이딩

## 다형성 
- 여러가지 형태의 성질을 가짐 
- 추상 클래스 : 여러 클래스들의 공통점을 추상화해서 모아놓은 클래스,  서로 관련있는 클래스들의 공통 부분을 묶어서 추상화함
   *  @abstractmethod 데코레이터가 없는 일반적인 메소드가 있어도 상관없음, 이 메소드들 또한 자식 클래스가 물려받아 그대로 사용하거나 오버라이딩하여 사용할 수 있음
   * @property가 붙으면 이런 구문들이 인스턴스 변수 x의 값을 직접 읽는다는 원래의 뜻이 아니라 getter 메소드 x를 실행한다는 의미로 바뀜
